...
function getAccountInstance (plugin) {
  if (!plugin) return
  ...
  if (_.isEmpty(activeInstances)) {
    throw new Error(`No active service configuration for plugin '${plugin?.code}'. Please check your 3rd party services options`)
  }

  if (_.size(activeInstances) > 1) {
    throw new Error(`Plugin '${plugin?.code}' has two or more active instances. Please make sure only a single instance is active at any given time`)
  }
