...
const electrum = require('./plugins/wallet/electrum/electrum')
...
const ELECTRUM_CONSUMED_ADDRESSES_INTERVAL = T.hour
...
function doPolling (schema) {
  ...
  activeWallet('BTC').then(wallet => {
    if (wallet === 'electrum')
      addToQueue(electrum.checkConsumedAddresses, ELECTRUM_CONSUMED_ADDRESSES_INTERVAL, schema, QUEUE.SLOW)
  })
}
...
const activeWallet = cryptoCode =>
  settingsLoader.loadLatest()
    .then(_.flow(
      _.get('config'),
      config => configManager.getWalletSettings(cryptoCode, config),
      _.get('wallet'),
    ))
