...
const checkUserPermissions = (user, permissionsRequired) => {
  return getUserPermissions(user.id)
    .then(res => {
      console.log('permissionsRequired', permissionsRequired)
      console.log('shouldAuthorize', _.size(_.difference(permissionsRequired, res.permissions)) === 0)
      return _.size(_.difference(permissionsRequired, res.permissions)) === 0
    })
    .catch(() => false)
}
