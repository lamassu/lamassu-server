...
const buildTriggers = async (allTriggers) => {
  const normalTriggers = []
  const customTriggers = _.filter(o => {
    if (o.customRequirementId === '') normalTriggers.push(o)
    return o.customRequirementId !== ''
  }, allTriggers)
  const customTriggerDetails = await _.flow([_.map(_.get('customRequirementId')), customRequestQueries.batchGetCustomInfoRequest])(customTriggers)

  customTriggerDetails.forEach((details, index) => {
    customTriggers[index] = { ...customTriggers[index], customInfoRequest: details }
  })

  // test, remove before PR
  customTriggers.forEach(trigger => {
    if (trigger.customRequirementId !== trigger.customInfoRequest.id) console.log('WRONG CUSTOM INFO REQUEST', trigger)
  })
