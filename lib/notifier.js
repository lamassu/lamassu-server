      ...
      return checkPings(devices)
        .then(pings => {
          ...
          devices.forEach(function (device) {
            ...
            const stuckScreen = _.map(function (alert) {
              return _.set('machineName', deviceName, alert)
...
function alertSubject (alertRec) {
  ...
  const alertTypes = _.map(function (el) {
    return {
      codeDisplay: codeDisplay(el.code),
      machineName: el.machineName
    }
  }, alerts).sort()

  const displayAlertTypes = _.uniq(_.map(function (alertType) {
    return alertType.codeDisplay + ' (' + alertType.machineName + ') '
  }, alertTypes))
