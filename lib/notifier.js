...
function alertSubject (alertRec) {
  ...
  const alertTypes = _.map(entry => {
    const code = entry[0]
    const machineNames = _.map(el => el.machineName, entry[1])
  ...
  const displayAlertTypes = _.uniq(_.map(alertType => {
    if (alertType.machineNames && alertType.machineNames.length) return alertType.codeDisplay + ' (' + alertType.machineNames.join(', ') + ') '
    return alertType.codeDisplay
  }, sortedAlertTypes))
