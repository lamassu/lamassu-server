...
async function updateCustomer (id, data, userToken) {
  ...
  if (data.subscriberInfo) {
    await Promise.all([getCustomers({ id }), settingsLoader.loadLatest()])
...
function edit (id, data, userToken) {
  ...
  if (_.isEmpty(filteredData)) return getCustomers({ id })
...
function getCustomers (opts) {
  ...
  const { id, phone = null, name = null, address = null, idCardDataID = null } = opts
  ...
  FROM (
    ...
    ${!_.isNil(id) ? `WHERE c.id = $2` : `WHERE c.id != $2`}
  ) AS cl WHERE rn = 1
  ${!_.isNil(id)
    ? ``
  ...
  const standardizeCustomer = customer => _.reduce((acc, value) => acc.then(value), Promise.resolve(customer), [assignCustomerData, getCustomInfoRequestsData, camelizeDeep, formatSubscriberInfo])

  return !_.isNil(id)
    ? db.oneOrNone(sql, [passableErrorCodes, id]).then(standardizeCustomer)
    : db.any(sql, [ passableErrorCodes, anonymous.uuid, NUM_RESULTS, phone, name, address, idCardDataID ])
      .then(customers => Promise.all(_.map(standardizeCustomer, customers)))
}
