...
function findById (id) {
  const sql = 'SELECT id, username, role, last_accessed, hardware_credentials FROM users WHERE id=$1 limit 1'
  return db.oneOrNone(sql, [id])
}

function get2FAMethod (id) {
  const sql = `SELECT id, username, twofa_code, hardware_credentials->>'devices' AS devices FROM users WHERE id=$1`
  return db.oneOrNone(sql, [id])
}
...
function saveHardwareDevices (id, devices) {
  devices.map(dev => {
    console.log(Buffer.from(dev.credentialID).toString())
  })
