...
/**
* @api {post} /machines/:machineId/actions/:action
* @apiName Do :action on specified :machineId
*
* @apiParam  {String} [machineId] machine
* @apiParam  {String} [action] action
*/
app.post('/api/machines/:machineId/actions/:action', (req, res, next) => {
  const actions = ['unpair', 'reboot']
  const { machineId: machine, action } = req.params
  ...
  machineLoader.getMachines()
    .then(machines => machines.find(({ deviceId }) => deviceId === machine))
    .then(found => found ? next() : res.status(404).send({
      statusCode: 404,
      error: 'Not Found',
      message: `:machine ${machine} does not exists`
    }))
})

app.post('/api/machines/:machineId/actions/:action', (req, res) => {
  const { machineId: deviceId, action } = req.params

  machineLoader.setMachine({ deviceId, action })
    .then(() => machineLoader.getMachineNames())
    .then(machines => machines.find(m => m.deviceId === deviceId))
