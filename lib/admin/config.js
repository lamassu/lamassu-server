...
function fetchConfigGroup (code, devMode) {
  ...
  return Promise.all([fetchSchema(), fetchData(), fetchConfig(), fetchMachines()])
    .then(([schema, data, config, machineList]) => {
      ...
      if (devMode) {
        const allowed = ['mock-ticker', 'mock-wallet', 'mock-exchange', 'mock-sms', 'mock-id-verify', 'mock-zero-conf']
        const isValid = o => _.includes(o.code, allowed)
        data.accounts = _.filter(isValid, data.accounts)
      }
