  ...
  return cashInAtomic.atomic(machineTx, pi)
    .then(r => {
      ...
      return Promise.all([settingsLoader.loadLatest(), checkForBlacklisted(updatedTx), doesTxReuseAddress(updatedTx), getWalletScore(updatedTx, pi)])
        ...
        .catch(err => {
          logger.info(`Caught an error on cash-in post function ${err}`)
          throw err
        })
    })
}
...
function postProcess (r, pi, isBlacklisted, addressReuse, walletScore) {
  ...
  return pi.sendCoins(r.tx)
    .then(txObj => {
      if (txObj.batched) {
        logger.info(`Transaction is being batched`)
        return {
          batched: true,
          batchTime: 'now()^',
          sendPending: true,
      ...
      logger.info(`Transaction is not being batched`)
      return {
        txHash: txObj.txid,
        fee: txObj.fee,
        sendConfirmed: true,
    ...
    .catch(err => {
      logger.info(`Caught error on cash-in postProcess: ${err}`)
