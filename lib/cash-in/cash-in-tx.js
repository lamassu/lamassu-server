...
function post (machineTx, pi) {
  ...
  return cashInAtomic.atomic(machineTx, pi)
    .then(r => {
      ...
      let walletScore = {}
      let testingAddress = false

      return Promise.all([settingsLoader.loadLatest(), checkForBlacklisted(updatedTx), doesTxReuseAddress(updatedTx), getWalletScore(updatedTx, pi), checkForTestingAddress(updatedTx)])
        .then(([{ config }, blacklistItems, isReusedAddress, fetchedWalletScore, isTestingAddress]) => {
          const rejectAddressReuse = configManager.getCompliance(config).rejectAddressReuse

          walletScore = fetchedWalletScore
          testingAddress = isTestingAddress
        ...
        .then(tx => _.set('isTestTransaction', testingAddress, tx))
    })
}
...
function checkForTestingAddress (tx) {
  // Check only on addressScan and avoid testing for testing address on every bill inserted
  if (!tx.fiat || tx.fiat.isZero()) {
    return ta.testing(tx.toAddress, tx.cryptoCode).then(as => as.length === 1)
  }
  return Promise.resolve(false)
}
