...
function atomic (machineTx, pi) {
  ...
  function transaction (t) {
    ...
    const sql3 = 'select id from cashbox_sessions where device_id=$1 ORDER BY device_time DESC LIMIT 1'
    ...
    return t.oneOrNone(sql, [machineTx.id])
      .then(row => {
        ...
        return Promise.all([
          t.any(sql2, [machineTx.id]),
          t.oneOrNone(sql3, [machineTx.deviceId])
        ])
          .then(([billRows, cashboxSessionId]) => {
            ...
            return preProcess(dbTx, machineTx, pi)
              ...
              .then(r => {
                return insertNewBills(t, billRows, machineTx, cashboxSessionId)
...
function insertNewBills (t, billRows, machineTx, cashboxSessionId) {
  const bills = pullNewBills(billRows, machineTx)
  if (_.isEmpty(bills)) return Promise.resolve([])

  const dbBills = _.map((bill) => _.set('cashbox_session_id', cashboxSessionId, bill), _.map(cashInLow.massage, bills))
