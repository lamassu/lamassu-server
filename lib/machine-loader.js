...
function createMachineLocation ({ id, label, addressLine1, addressLine2, zipCode, country }) {
  ...
  return db.none(sql, [id, label, addressLine1, addressLine2, zipCode, country])
}

function deleteMachineLocation (id) {
  return db.tx(t => {
    const q1 = t.none(`DELETE FROM machine_locations WHERE id = $1`, [id])
    const q2 = t.none(`UPDATE devices SET machine_location = NULL WHERE machine_location = $1`, [id])
    const q3 = t.none(`UPDATE pairing_tokens SET machine_location = NULL WHERE machine_location = $1`, [id])
