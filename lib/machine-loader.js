...
function getMachinesUpdateStatus () {
  const sql = `SELECT MAX(created), event, device_id FROM machine_updates GROUP BY device_id, event`
  return db.manyOrNone(sql)
    .then(res => _.map(_.mapKeys(_.camelCase))(res))
}

function getUpdateStatus (deviceId, latest) {
  const sql = `SELECT * FROM machine_updates WHERE device_id = $1 ORDER BY created DESC`
  return db.manyOrNone(sql, [deviceId])
    .then(result => {
      if (latest && result) return _.head(result)
      return result
    })
}

function recordUpdateEvent (deviceId, event) {
  return db.oneOrNone('SELECT * FROM devices WHERE device_id=$1', [deviceId])
    .then(it => {
      const dbData = {
        ...
        device_id: deviceId,
        event: event,
        new_version: '10.0.0', // TODO: Fetch new version available
