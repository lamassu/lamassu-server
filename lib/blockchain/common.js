...
const BINARIES = {
  ...
  ADA: {
    url: 'https://github.com/input-output-hk/cardano-sl/archive/3.1.0.tar.gz',
    build: {folder: 'cardano-sl-3.1.0', script: 'cardano-build.sh'},
    dir: 'cardano-sl-3.1.0',
    files: [['cardano-testnet-wallet', 'cardano-testnet-wallet']]
  }
}
...
function fetchAndInstall (coinRec) {
  ...
  if (binaries.build) {
    let res = es(`${__dirname}/${binaries.build.script} \`realpath ${binaries.build.folder}\``)
    console.log(res);
  }

  if (_.isEmpty(binaries.files) && binDir) {
