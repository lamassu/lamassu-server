...
function setupCrypto (crypto) {
  ...
  if (process.env.NODE_ENV === 'development') {
    if (_.isNil(process.env[`${crypto.cryptoCode}_NODE_IP`]))
      throw new Error(`The environment variable for ${crypto.cryptoCode}_NODE_IP is not set!`)

    if (_.isNil(process.env.BLOCKCHAIN_DIR))
  ...
  const tmpDir = process.env.NODE_ENV === 'development' ? path.resolve(BLOCKCHAIN_DIR, 'tmp', 'blockchain-install') : '/tmp/blockchain-install'
...
function getBlockchainSyncStatus (cryptoList) {
  return settingsLoader.loadLatest()
    .then(settings => {
      if (process.env.NODE_ENV === 'development') return new Array(_.size(cryptoList)).fill('ready')
...
function run () {  
  const choices = _.flow([
    ...
    _.map(c => {
      const checked = process.env.NODE_ENV === 'development' ? isInstalledSoftware(c) : isInstalledSoftware(c) && isInstalledVolume(c)
