...
function run () {
  ...
  const choices = _.map(c => {
    ...
    return {
      ...
      checked,
      disabled: z => {
        if (checked || c.cryptoCode.toString() === ETH_CRYPTO_CODE)
          return 'Installed, use Infura option'
      }
    }
  }, cryptos)
  ...
  inquirer.prompt(questions)
    .then(answers => {
      answers.crypto.push(coinUtils.getCryptoCurrency(ETH_CRYPTO_CODE).code)
