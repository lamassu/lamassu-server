...
class Layer2Composite {
  constructor (walletAdapter, layer2, account) {
  ...
  newAddress (settings, info) {
    return Promise.all([
      this.layer2.newAddress(settings, info),
  ...
  getLayer2Status (settings, tx) {
    ...
    return this.layer2.getStatus(this.account, toAddress, tx.cryptoAtoms, tx.cryptoCode)
  }
