...
function getAvailableIndividualDiscountsWithCustomerData () {
  const sql = `
  SELECT 
    individual_discounts.id,
    individual_discounts.discount,
    customers.phone,
    customers.id_card_data
  FROM individual_discounts
  LEFT JOIN customers 
  ON individual_discounts.customer_id=customers.id
  WHERE individual_discounts.soft_deleted=false`
  return db.any(sql).then(_.map(_.mapKeys(_.camelCase)))
}
...
module.exports = {
  ...
  getAvailableIndividualDiscountsWithCustomerData,
