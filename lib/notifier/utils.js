...
const isActive = _.any(it => it.active)

const isChannelActive = channel => list => !_.isEmpty(_.filter(it => it.active && it.channel === channel)(list))

const isCategoryActive = category => list => !_.isEmpty(_.filter(it => it.active && it.category === category)(list))
...
function buildAlertFingerprint (alertRec, notifications) {
  const sms = getAlertTypes(alertRec, _.filter(it => it.channel === 'sms')(notifications.preferences))
  const email = getAlertTypes(alertRec, _.filter(it => it.channel === 'email')(notifications.preferences))
...
function deviceAlerts (notifications, alertRec, device) {
  let alerts = []
  if (isCategoryActive('balance')(notifications)) {
    alerts = _.concat(alerts, alertRec.devices[device].balanceAlerts)
  }

  if (isCategoryActive('errors')(notifications)) {
    alerts = _.concat(alerts, alertRec.devices[device].deviceAlerts)
  }
  return alerts
}

function getAlertTypes (alertRec, notifications) {
  let alerts = []
  if (!isActive(notifications)) return alerts
  ...
  _.forEach(device => {
    alerts = _.concat(alerts, deviceAlerts(notifications, alertRec, device))
