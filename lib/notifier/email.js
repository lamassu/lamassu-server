...
function alertSubject (alertRec, notifications) {
  let alerts = []

  if (utils.isCategoryActive('balance')) {
    alerts = _.concat(alerts, alertRec.general)
  }

  _.forEach(device => {
    alerts = _.concat(alerts, utils.deviceAlerts(notifications, alertRec, device))
...
function printEmailAlerts (alertRec, notifications) {
  let body = 'Errors were reported by your Lamassu Machines.\n'

  if (utils.isCategoryActive('balance') && alertRec.general.length !== 0) {
    body += '\nGeneral errors:\n'
  ...
  _.forEach(device => {
    ...
    const alerts = utils.deviceAlerts(notifications, alertRec, device)
