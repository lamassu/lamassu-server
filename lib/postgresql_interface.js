...
exports.machineEvent = function machineEvent (rec) {
  const fields = ['id', 'device_id', 'event_type', 'note', 'device_time', 'cashbox_session_id']
  const sql = getInsertQuery('machine_events', fields)
  const values = [rec.id, rec.deviceId, rec.eventType, rec.note, rec.deviceTime, rec.cashboxSessionId]
...
exports.cashboxSessionInsert = function cashboxInsert (rec) {
  const fields = ['id', 'device_id', 'device_time']
  const sql = getInsertQuery('cashbox_sessions', fields)
  const values = [rec.id, rec.deviceId, rec.deviceTime]
