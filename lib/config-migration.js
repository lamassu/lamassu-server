...
function migrateCommissions (config) {
  ...
  let allCommissionsOverrides = withCryptoScoped.concat(filteredMachineScoped)
...
function migrateComplianceTriggers (config) {
  ...
  let triggers = []
  if (global.smsVerificationActive)
    triggers.push(
      createTrigger(requirements.sms, global.smsVerificationThreshold)
    )
  if (global.idCardDataVerificationActive)
    triggers.push(
      createTrigger(requirements.idData, global.smsVerificationThreshold)
    )
  if (global.idCardPhotoVerificationActive)
    triggers.push(
      createTrigger(requirements.idPhoto, global.smsVerificationThreshold)
    )
  if (global.frontCameraVerificationActive)
    triggers.push(
      createTrigger(requirements.facePhoto, global.smsVerificationThreshold)
    )
  if (global.sanctionsVerificationActive)
    triggers.push(
      createTrigger(requirements.sanctions, global.smsVerificationThreshold)
    )
