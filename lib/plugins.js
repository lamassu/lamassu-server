...
function plugins(settings, deviceId) {
  ...
  function notifyOperator(tx, rec) {
    ...
    const notificationsEnabled =
      notifications.sms.transactions || notifications.email.transactions
    const highValueTx = tx.fiat.gt(
      notifications.highValueTransaction || Infinity
    )
  ...
  function sendTransactionMessage(rec) {
    ...
    const emailActive =
      notifications.email.active && notifications.email.transactions
