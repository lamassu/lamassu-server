...
function plugins (settings, deviceId) {
  ...
  function notifyOperator (tx, rec) {
    ...
    if (zeroConf && isCashOut && !rec.isRedemption) return Promise.resolve()

    if (!zeroConf && rec.isRedemption) return sendRedemptionMessage(tx.id, rec.error)

    return Promise.all([machineLoader.getMachineName(tx.deviceId), customers.getById(tx.customerId)])
