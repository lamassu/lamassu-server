...
function plugins (settings, deviceId) {
  ...
  function recordPing (serialNumber, deviceTime, rec) {
    ...
    return Promise.all([
      db.none('insert into machine_pings(id,device_id,serial_number,device_time) values($1, $2, $3, $4) ON CONFLICT (device_id) DO UPDATE SET device_time = $4', [machinePings.id, deviceId, serialNumber, deviceTime]),
