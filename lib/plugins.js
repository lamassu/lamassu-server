...
function plugins (settings) {
  function buildRates (deviceId, tickers) {
    ...
    console.log('DEBUG44: %j', [cashOutCommission, config.cashOutCommission])
    const rates = {}
  ...
  function pollQueries (deviceTime, deviceId, deviceRec) {
    ...
    const cassetteCounts = argv.cassettes

    const cassettePromise = cassetteCounts
    ? Promise.resolve({counts: cassetteCounts.split(',')})
