...
function plugins (settings, deviceId) {
  ...
  function sendMessage (rec) {
    return notifications.getNotificationChannelPreferences()
      .then(preferences => {
        let promises = []
        if (_.find(it => it.channel === 'email')(preferences).active && rec.email) promises.push(email.sendMessage(settings, rec))
        if (_.find(it => it.channel === 'sms')(preferences).active && rec.sms) promises.push(sms.sendMessage(settings, rec))

        return Promise.all(promises)
      })
  }

  function checkDevicesCashBalances (fiatCode, devices, { settings, preferences }) {
    const defaultNotificationSettings = _.find(it => it.overrideId === NIL)(settings)
    const getDeviceNotificationSettings = device => _.defaultTo(
      defaultNotificationSettings,
      _.find(it => it?.value?.machine === device.deviceId)(settings)
    ).value
    return _.map(device => checkDeviceCashBalances(fiatCode, device, { settings: getDeviceNotificationSettings(device), preferences }), devices)
  }

  function checkDeviceCashBalances (fiatCode, device, notifications) {
    ...
    const cashboxAlert = device.cashUnits.cashbox > notifications.settings.cashboxCount.upperBound
      ? {
        code: 'CASH_BOX_FULL',
        machineName,
        deviceId: device.deviceId,
        notes: _.defaultTo(0, device.cashUnits.cashbox)
      }
      : null

    const rejectBoxAlert = _.defaultTo(0, device.cashUnits.rejectBox) > _.defaultTo(0, notifications.settings.rejectBoxCount.upperBound)
      ? {
        code: 'REJECT_BOX_FULL',
        machineName,
        deviceId: device.deviceId,
        notes: _.defaultTo(0, device.cashUnits.rejectBox)
      }
      : null

    const loadboxAlerts = _.map(it => {
      if (device.model !== 'aveiro') return null

      if (device.numberOfCassettes >= it && isUnitLow(device.cashUnits[`cassette${it}`], CASH_UNIT_CAPACITY[device.model]['cassette'], notifications.settings.loadboxPercentage.lowerBound[it - 1])) {
        return {
          code: 'LOW_LOAD_BOX',
          cassette: it + 1,
          machineName,
          deviceId: device.deviceId,
          notes: device.cashUnits[`cassette${it}`],
          denomination: cashOutConfig[`cassette${it}`],
          fiatCode
        }
      }

      return null
    })(_.range(1, _.defaultTo(0, _.size(notifications.settings.loadboxPercentage.lowerBound)) + 1))

    const cassetteAlerts = _.flatMap(it => {
      const res = []
      if (device.model !== 'aveiro') {
        if (device.numberOfCassettes >= it && isUnitLow(device.cashUnits[`cassette${it}`], CASH_UNIT_CAPACITY[device.model]['cassette'], notifications.settings.cassetteAndRecyclerPercentage.lowerBound[it - 1])) {
          res.push({
            code: 'LOW_CASSETTE',
            cassette: it,
            machineName,
        ...
        if (device.numberOfStackers >= it && isUnitLow(device.cashUnits[`stacker${it}f`], CASH_UNIT_CAPACITY[device.model]['stacker'], notifications.settings.cassetteAndRecyclerPercentage.lowerBound[it - 1])) {
          res.push({
            code: 'LOW_STACKER',
            cassette: it,
            machineName,
            deviceId: device.deviceId,
            notes: device.cashUnits[`stacker${it}f`],
            denomination: cashOutConfig[`stacker${it}f`],
            fiatCode
          })
        }

        if (device.numberOfStackers >= it && isUnitLow(device.cashUnits[`stacker${it}r`], CASH_UNIT_CAPACITY[device.model]['stacker'], notifications.settings.cassetteAndRecyclerPercentage.lowerBound[it - 1])) {
          res.push({
            code: 'LOW_STACKER',
            cassette: it,
            machineName,
            deviceId: device.deviceId,
            notes: device.cashUnits[`stacker${it}r`],
            denomination: cashOutConfig[`stacker${it}r`],
            fiatCode
          })
        }
      }
      ...
      return res
    })(_.range(1, _.defaultTo(0, _.size(notifications.settings.cassetteAndRecyclerPercentage.lowerBound)) + 1))
  ...
  function checkCryptoBalances (fiatCode, devices, notifications) {
    ...
    const defaultNotificationSettings = _.find(it => it.overrideId === NIL)(notifications.settings)
    const getCryptoNotificationSettings = cryptoCode => _.defaultTo(
      defaultNotificationSettings,
      _.find(it => it?.value?.cryptoCurrency === cryptoCode)(notifications.settings)
    ).value

    return Promise.all(fiatBalancePromises(cryptoCodes))
      .then(balances => _.map(it => checkCryptoBalanceWithFiat(it, getCryptoNotificationSettings(it[0])), _.zip(cryptoCodes, balances)))
  }

  function checkCryptoBalance (fiatCode, rec, notifications) {
    const [cryptoCode, fiatBalance] = rec
    if (!fiatBalance) return null

    const lowAlertThreshold = notifications.lowerBound
    const highAlertThreshold = notifications.upperBound
  ...
  function checkBalances (notifications) {
    const localeConfig = configManager.getGlobalLocale(settings.config)
    const fiatCode = localeConfig.fiatCurrency

    const cryptoBalanceNotifications = _.mapValues(it => _.filter(ite => ite.event === 'cryptoBalance')(it))(notifications)
    const fiatBalanceNotifications = _.mapValues(it => _.filter(ite => ite.event === 'unitFillThreshold')(it))(notifications)

    return machineLoader.getMachines()
      .then(devices => Promise.all([
        checkCryptoBalances(fiatCode, devices, cryptoBalanceNotifications),
        checkDevicesCashBalances(fiatCode, devices, fiatBalanceNotifications)
      ]))
