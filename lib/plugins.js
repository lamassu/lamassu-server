...
function plugins (settings, deviceId) {
  ...
  function computeAvailableCassettes (cassettes, redeemableTxs) {
    const sumTxs = (sum, tx) => {
      const expectedFiat = tx.provisioned1 * cassettes[0].denomination +
        tx.provisioned2 * cassettes[1].denomination

      if (!tx.fiat.eq(expectedFiat)) {
        throw new Error('Provisioned notes don\'t add up, cassettes were changed.')
      }

      return {
        provisioned1: sum.provisioned1 + tx.provisioned1,
        provisioned2: sum.provisioned2 + tx.provisioned2
      }
    }

    const provisioned = _.reduce(sumTxs, redeemableTxs)
    const counts = [
      cassettes[0].count - provisioned.provisioned1,
      cassettes[1].count - provisioned.provisioned2
    ]

    if (_.some(_.lt(0), counts)) {
      throw new Error('Negative note count.')
    }
  ...
  function buildAvailableCassettes () {
    const config = configManager.machineScoped(deviceId, settings.config)
    ...
    return Promise.all([dbm.cassetteCounts(deviceId), cashOutHelper.redeemableTxs(deviceId)])
    .then(([rec, redeemableTxs]) => {
