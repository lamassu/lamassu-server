...
function plugins (settings, deviceId) {
  ...
  function computeAvailableCassettes (cassettes, redeemableTxs) {
    const availableCassetteStep = (acc, tx) => {
      const _cassettes = _.isArray(acc)
      ? acc
      : cassettes

      const notes = billMath.makeChange(_cassettes, tx.fiat)
    ...
    const fullReduce = _.reduce.convert({cap: false})
    fullReduce(availableCassetteStep, redeemableTxs, cassettes)
  ...
  function buildAvailableCassettes () {
    ...
    .then(([rec, redeemableTxs]) => {
      ...
      return computeAvailableCassettes(cassettesRec.cassettes, redeemableTxs)
    })
  }
