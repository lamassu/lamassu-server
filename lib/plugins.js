...
function plugins (settings, deviceId) {
  ...
  */
  ...
  function checkCryptoBalances () {
    const globalConfig = configManager.unscoped(settings.config)
    const fiatCode = globalConfig.fiatCurrency

    const fiatBalancePromises = cryptoCodes => cryptoCodes.map(c => fiatBalance(fiatCode, c))
    ...
    return machineLoader.getMachines()
    .then(devices => {
      const union = _.flow(_.uniq, _.flatten, _.map(fetchCryptoCodes))
  ...
  function checkBalance (rec) {
    ...
    const fiatBalance = rec.fiatBalance

    return BN(fiatBalance.balance).lt(cryptoAlertThreshold)
    ? {code: Symbol('LOW_BALANCE'), cryptoCode, fiatBalance, fiatCode: rec.fiatCode}
    : null
  }

  function checkBalances () {
    return checkCryptoBalances()
    .then(_.map(checkBalance))
  }
