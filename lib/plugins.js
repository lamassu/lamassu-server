...
function plugins (settings, deviceId, schema) {
  ...
  function buildAvailableCassettes (excludeTxId) {
    ...
    return Promise.all([dbm.cassetteCounts(deviceId), cashOutHelper.redeemableTxs(deviceId, excludeTxId)])
      .then(([rec, _redeemableTxs]) => {
        ...
        const denominations = _.map(i => cashOutConfig[`cassette${i}`], _.range(1, rec.numberOfCassettes+1))
        const virtualCassettes = [_.max(denominations) * 2, _.min(denominations) * 5]

        if (rec.counts.length !== denominations.length) {
          throw new Error('Denominations and respective counts do not match!')
        }

        const cassettes = _.map(([denomination, count]) => ({
          denomination: parseInt(denomination, 10),
          count: parseInt(count, 10)
        }), _.zip(denominations, rec.counts))
  ...
  function mapCoinSettings (coinParams) {
    ...
    const hasCashOutConstraints = configManager.getCashOutConstraints(cryptoCode, settings.config)

    return {
      cryptoCode,
      display: cryptoRec.display,
      isCashInOnly: Boolean(cryptoRec.isCashinOnly) || Boolean(hasCashOutConstraints),
  ...
  function fiatBalance (fiatCode, cryptoCode) {
    ...
    return Promise.all([
      ...
      wallet.balance(settings, cryptoCode, schema)
    ])
