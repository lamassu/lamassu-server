...
function fetchWallet (settings, cryptoCode) {
  return fs.readFile(MNEMONIC_PATH, 'utf8')
    .then(mnemonic => {
      ...
      const rawAccount = ph.getAccountInstance(settings.accounts[plugin], plugin)
...
function newFunding (settings, cryptoCode, options) {
  return fetchWallet(settings, cryptoCode)
    .then(r => {
      const wallet = r.wallet
      const account = r.account

      return wallet.newFunding(account, cryptoCode, settings, r.operatorId, options)
    })
}
...
function authorizeZeroConf (settings, tx, machineId) {
  ...
  const account = ph.getAccountInstance(settings.accounts[plugin], plugin)
    ...
    .then(multiplier => {
      switch (multiplier) {
        case BALANCE_FETCH_SPEED_MULTIPLIER.NORMAL:
          return balanceNormal(settings, cryptoCode, schema)
        case BALANCE_FETCH_SPEED_MULTIPLIER.SLOW:
          return balanceSlow(settings, cryptoCode, schema)
        default:
          throw new Error()
      }
    })
...
const balanceNormal = mem(_balance, {
  ...
  cacheKey: (settings, cryptoCode, schema) => `${schema}:${cryptoCode}`
})
