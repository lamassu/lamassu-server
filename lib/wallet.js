...
const WalletAdapter = require('./wallet-adapter')
...
function fetchWallet (settings, cryptoCode) {
  return fs.readFile(options.mnemonicPath, 'utf8')
    .then(mnemonic => {
      ...
      const adapter = new WalletAdapter(wallet, account)

      return layer2.fetchLayer2(settings, cryptoCode)
        .then(fetchedLayer2 => {
          if (!fetchedLayer2) return { wallet: adapter, account }

          const composite = new layer2.Layer2Composite(adapter, fetchedLayer2, account)

          return { wallet: composite, account }
        })
    })
}
...
function newAddress (settings, info) {
  return fetchWallet(settings, info.cryptoCode)
    .then(r => r.wallet.newAddress(settings, info))
}
...
function getStatus (settings, tx, machineId) {
  return fetchWallet(settings, tx.cryptoCode)
    .then(r => r.wallet.getStatus(settings, tx))
    .then((statusRec) => {
