...
function newFunding (settings, cryptoCode, addresses) {
  return fetchWallet(settings, cryptoCode)
    .then(r => {
      const wallet = r.wallet
      const account = r.account

      return wallet.newFunding(account, cryptoCode, settings, r.operatorId, addresses)
    })
}
...
const balance = (settings, cryptoCode, schema) => {
  if (_.includes(coinFilter, cryptoCode)) return balanceFiltered(settings, cryptoCode, schema)
  return balanceUnfiltered(settings, cryptoCode, schema)
}
