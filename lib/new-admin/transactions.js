...
function addCustomers (txs) {
  const addCustomer = tx => customers.getById(tx.customerId)
    .then(c => {
      return _.set('customer', c, tx)
    })

  return Promise.all(_.map(addCustomer, txs))
}

function addTxHashCashOut (txs) {
  const addHash = tx => {
    const sql = `select tx_hash from cash_out_actions where (tx_id = $1)`

    return Promise.all([db.many(sql, [tx.id])])
      .then(h => {
        console.log(h[0][0])
        return _.set('txHash', h[0][0].tx_hash, tx)
      })
  }

  return Promise.all(_.map(addHash, txs))
}
...
function batch () {
  ...
  const cashOutSql = `select 'cashOut' as tx_class, txs.*,
  actions.tx_hash,
  (extract(epoch from (now() - greatest(txs.created, txs.confirmed_at))) * 1000) >= $2 as expired
  from cash_out_txs txs, cash_out_actions actions where txs.id = actions.tx_id and actions.action = 'provisionAddress'
