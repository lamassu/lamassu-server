...
function authenticate (token, ua) {
  const sql = 'select token from user_tokens where token=$1'
  const sqlUpdateAccess = `update user_tokens set user_agent=$1, last_accessed = now() where token=$2`

  return db.one(sql, [token]).then(() => db.none(sqlUpdateAccess, [ua, token])).then(() => true).catch(() => false)
}
