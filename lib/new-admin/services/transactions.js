...
function getCashoutAddresses (cryptoCode) {
  const sql = `SELECT to_address FROM cash_out_txs WHERE crypto_code = $1`
  return db.any(sql, [cryptoCode])
    .then(_.flow(_.map(it => it.to_address), _.uniq))
}
