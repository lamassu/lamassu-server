...
function batch (
  ...
  const packager = _.flow(
    ...
    _.map(_.flow(
      ...
      _.mapKeys(k =>
        ...
        k == 'cashInFeeCrypto' ? 'fixedFeeCrypto' :
        k
      )
    )),
    addProfits,
  ...
  array_to_string(array[ec.id_card_data::json->>'firstName', ec.id_card_data::json->>'lastName'], ' ') AS edited_customer_name,
  ...
  LEFT OUTER JOIN edited_customer_data ec ON txs.customer_id = ec.customer_id
  ...
  AND ($14 is null or txs.updated_at >= $14)
  ...
  if (hasCashInOnlyFilters) {
    promises = [db.any(cashInSql, [cashInTx.PENDING_INTERVAL, from, until, limit, offset, id, txClass, machineName, customerName, fiatCode, cryptoCode, toAddress, status, updatedSince])]
  } else if (hasCashOutOnlyFilters) {
    promises = [db.any(cashOutSql, [REDEEMABLE_AGE, from, until, limit, offset, id, txClass, machineName, customerName, fiatCode, cryptoCode, toAddress, status, swept, updatedSince])]
    promises = [db.any(cashOutSql, [REDEEMABLE_AGE, from, until, limit, offset, id, txClass, machineName, customerName, fiatCode, cryptoCode, toAddress, status, swept, updatedSince])]
  } else {
    promises = [
      db.any(cashInSql, [cashInTx.PENDING_INTERVAL, from, until, limit, offset, id, txClass, machineName, customerName, fiatCode, cryptoCode, toAddress, status, updatedSince]),
      db.any(cashOutSql, [REDEEMABLE_AGE, from, until, limit, offset, id, txClass, machineName, customerName, fiatCode, cryptoCode, toAddress, status, swept, updatedSince])
    ]
  }

  return Promise.all(promises)
    .then(packager)
    .then(updateCustomerEditedData)
    .then(res =>
      !isCsvExport ? res :
...
function updateCustomerEditedData (data) {
  // TODO: should we update the other editable fields too?
  return _.map(it => ({
    ...it,
    customerName: it.editedCustomerName ?? it.customerName
  }))(data)
}
