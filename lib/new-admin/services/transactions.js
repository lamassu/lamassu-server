...
function batch (
  ...
  ${excludeTestingCustomers ? `AND c.is_test_customer IS FALSE AND txs.is_test_transaction IS FALSE` : ``}
...
function enableTestTransaction (txClass, txID) {
  const sql = `
    UPDATE ${txClass === 'cashIn' ? 'cash_in_txs' : 'cash_out_txs' } 
    SET is_test_transaction=TRUE
    WHERE id=$1`

  return db.oneOrNone(sql, [txID])
}

function disableTestTransaction (txClass, txID) {
  const sql = `
    UPDATE ${txClass === 'cashIn' ? 'cash_in_txs' : 'cash_out_txs' } 
    SET is_test_transaction=FALSE
    WHERE id=$1`
