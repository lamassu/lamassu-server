...
function batch (
  ...
  NOT (tst.address IS NULL AND tst.crypto_code IS NULL) AS is_test_transaction
  FROM (SELECT *, ${cashInTx.TRANSACTION_STATES} AS txStatus FROM cash_in_txs) AS txs
  LEFT OUTER JOIN testing_addresses AS tst ON tst.address = txs.to_address AND txs.crypto_code = tst.crypto_code
  ...
  ${excludeTestingCustomers ? `AND c.is_test_customer IS FALSE AND is_test_transaction IS FALSE` : ``}
