...
function toSmallestUnit (cryptoCode, amount) {
  const cryptoRec = coinUtils.getCryptoCurrency(cryptoCode)
  const unitScale = cryptoRec.unitScale

  return new BN(amount).shiftedBy(unitScale).decimalPlaces(0)
}

function getSingleCoinFunding (settings, fiatCode, cryptoCode) {
  return transactions.getCashoutAddresses(cryptoCode)
    .then(cashOutAddresses => {
      const promises = [
        wallet.newFunding(settings, cryptoCode, cashOutAddresses),
      ...
      return Promise.all(promises)
        .then(([fundingRec, ratesRec, batchRec]) => {
          ...
          const _walletHistory = fundingRec.walletHistory
          const walletHistory = _.map(it => ({ ...it, fiatValue: computeFiat(rate, cryptoCode, toSmallestUnit(cryptoCode, it.amount)) }), _walletHistory)
