...
const resolvers = {
  ...
  Query: {
    customers: (...[, { phone, name, address, id }]) => customers.getCustomers({ phone, name, address, idCardDataID: id }),
    customer: (...[, { customerId }]) => customers.getCustomers({ id: customerId }),
    customerFilters: () => filters.customer()
  },
  Mutation: {
    setCustomer: (root, { customerId, customerInput }, context, info) => {
      const token = authentication.getToken(context)
      if (customerId === anonymous.uuid) return customers.getCustomers({ id: customerId })
    ...
    replacePhoto: async (root, { customerId, photoType, newPhoto }, context) => {
      ...
      if (!photo) return customers.getCustomers({ id: customerId })
    ...
    deleteEditedData: (root, { customerId, customerEdit }) => {
      ...
      return customers.getCustomers({ id: customerId })
    },
