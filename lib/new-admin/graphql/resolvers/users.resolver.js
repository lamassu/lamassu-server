...
const resolver = {
  Query: {
    ...
    generateAttestationOptions: (...[, { userID }]) => authentication.generateAttestationOptions(userID),
    generateAssertionOptions: (...[, { username, password }]) => authentication.generateAssertionOptions(username, password)
  },
  Mutation: {
    ...
    validateAttestation: (...[, { userID, attestationResponse }]) => authentication.validateAttestation(userID, attestationResponse),
    validateAssertion: (root, args, context, info) =>
      authentication.validateAssertion(args.username, args.password, args.rememberMe, args.assertionResponse, context)
  }
}
