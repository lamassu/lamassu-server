...
const typeDef = gql`
  ...
  type Mutation {
    createPromoCode(code: String!, discount: Int!): PromoCode @auth(permissions: ["promoCodes:write"])
    deletePromoCode(codeId: ID!): PromoCode @auth(permissions: ["promoCodes:delete"])
    createIndividualDiscount(customerId: ID!, discount: Int!): IndividualDiscount @auth(permissions: ["individualDiscounts:write"])
