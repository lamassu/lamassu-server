...
const initializeSession = (context, user, rememberMe) => {
  const finalUser = { id: user.id, username: user.username }
...
const executeProtectedAction = (code, id, context, action) => {
  return users.getUserById(id)
    .then(user => {
      // TODO: This is currently checking for the superuser role name. But in the future, it should check for role permissions
...
const validateRegisterLink = token => {
  ...
  return users.validateUserRegistrationToken(token)
    .then(r => {
      ...
      return { username: r.username, roleId: r.roleId }
    })
}
...
const createRegisterToken = (username, roleId) => {
  return users.getUserByUsername(username)
    .then(user => {
      if (user) throw new authErrors.UserAlreadyExistsError()

      return users.createUserRegistrationToken(username, roleId)
    })
}

const register = (token, username, password, roleId) => {
  return users.getUserByUsername(username)
    .then(user => {
      if (user) throw new authErrors.UserAlreadyExistsError()
      return users.register(token, username, password, roleId).then(() => true)
    })
}
