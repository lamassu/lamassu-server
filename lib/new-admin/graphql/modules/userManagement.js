...
const queries = require('../../../ultralight/queries')
...
const createRegisterToken = (username, role, context) => {
  const operatorId = context.res?.locals?.operatorId ?? null

  if (_.isNil(operatorId))
    throw new AuthenticationError('Trying to create new user with a user with no operator associated!')

  return queries.createUser(username, operatorId).then(() => {
    return users.getUserByUsername(username)
      .then(user => {
...
const register = (token, username, password, role) => {
  return queries.registerUser(username).then(() => {
    return users.getUserByUsername(username)
      .then(user => {
        if (user) throw new authErrors.UserAlreadyExistsError()
