...
const login = (username, password) => {
  return authenticateUser(username, password).then(user => {
    ...
    return users.get2FAMethod(user.id).then(user => {
      const hardwareCredentials = user.devices
      const twoFASecret = user.twofa_code
      if (hardwareCredentials) return 'FIDO'
...
const generateAssertionOptions = (username, password) => {
  return authenticateUser(username, password).then(user => {
    return users.getHardwareDevices(user.id).then(devices => {
      const userDevices = JSON.parse(devices.devices)
...
const validateAttestation = (userID, attestationResponse) => {
  return users.getCurrentChallenge(userID).then(res => {
    const expectedChallenge = res.challenge
    ...
    return simpleWebauthn.verifyAttestationResponse({
      ...
      if (verified && attestationInfo) {
        ...
        const existingDevice = userDevices.find(device => device.credentialID === credentialID)
...
const validateAssertion = (username, password, rememberMe, assertionResponse, context) => {
  return authenticateUser(username, password).then(user => {
    return users.getCurrentChallenge(user.id).then(res => {
      const expectedChallenge = res.challenge

      return Promise.all([expectedChallenge, users.getHardwareDevices(user.id)])
    }).then(([expectedChallenge, devices]) => {
      const userDevices = JSON.parse(devices.devices)

      const dbAuthenticator = _.find(dev => {
        return Buffer.from(dev.credentialID).compare(base64url.toBuffer(assertionResponse.rawId)) === 0
      }, userDevices)
      ...
      const convertedAuthenticator = _.merge(
        dbAuthenticator,
        { credentialPublicKey: Buffer.from(dbAuthenticator.credentialPublicKey) }
      )
      ...
      if (verified) {
        console.log(convertedAuthenticator)
        convertedAuthenticator.counter = assertionInfo.newCounter
        console.log(convertedAuthenticator)
