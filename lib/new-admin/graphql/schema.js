...
const typeDefs = gql`
  ...
  type Query {
    ...
    userLatestToken: [UserToken]
    userSpecificTokens(user: String!): [UserToken]
  }
  ...
  type Mutation {
    ...
    revokeUserTokens(name: String!): [UserToken]
  }
`
...
const resolvers = {
  ...
  Query: {
    ...
    userLatestToken: () => tokenManager.getLatestTokenByUser(),
    userSpecificTokens: (... [, { name }]) => tokenManager.getSpecificUserTokens(name)
  },
  Mutation: {
    ...
    revokeUserTokens: (...[, { name }]) => tokenManager.revokeUserTokens(name)
  }
}
