...
const filterEmptyIndices = type => _.filter(idx => !_.get([type, idx], EMPTY_INDICES))
...
const fetchAllUTXOs = network =>
  freeAddressIndices()
    .then(_.flow(
      nonEmptyIndices,
      _.update('internal', _.map(c => _getInternalAddressByIndex(c, network))),
      _.update('external', _.map(c => _getExternalAddressByIndex(c, network))),
      ({ internal, external }) => _.uniq(_.concat(internal, external)),
      addresses => getUTXOsBatch(addresses, network)
    ))
