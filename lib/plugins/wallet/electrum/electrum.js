...
const electrumPromise = electrumConsts
  .then(({ host, port, protocol }) => {
    ...
    electrum.initElectrum({ client: "someclient x.y.z", version: "1.4" })
    return electrum
  })
...
const freeAddressIndeces = () =>
  helpers.freeAddressIndeces('BTC', wallet_hash, ['internal', 'external'])

/* Retrieves the next {in,ex}ternal free index */
const _nextFreeAddressIndex = (type) => () => freeAddressIndeces().then(_.get(type))
...
/*
 * Computes the {in,ex}ternal addresses of the next free indeces. External
...
const scriptHashFromAddress = (address, network) => _.flow(
...
const xpub2zpub = xpub => {
  let data = b58.decode(xpub)
  data = data.slice(4) // Drop the fingerprint?
  data = Buffer.concat([Buffer.from('04b24746', 'hex'), data])
  return b58.encode(data)
}

const get_pub = network => {
  const xpub = get_xpub(network)
  const zpub = xpub2zpub(xpub)
  return { xpub, zpub }
}
...
const fetchAllUTXOs = network =>
  freeAddressIndeces()
    .then(({ internal, external }) =>
      _.uniq(_.concat(
        _.map(c => _getInternalAddressByIndex(c, network), _.range(0, internal + GAP_LIMIT)),
        _.map(c => _getExternalAddressByIndex(c, network), _.range(0, external + GAP_LIMIT)),
      )))
    .then(addresses => getUTXOsBatch(addresses, network))
...
const getAddressBalance = address =>
  ...
  .then(({ electrum, network }) =>
    electrum.blockchainScripthash_getBalance(scriptHashFromAddress(address, network)))
  .then(_.flow(
...
const getUTXOsBatch = (addresses, network) => {
  const scriptHashes = _.map(address => scriptHashFromAddress(address, network), addresses)
  const scriptHash2addr = _.fromPairs(_.zip(scriptHashes, addresses))
      ...
      .then(_.flow(
        ...
        _.flatMap(({ param: scriptHash, result }) => _.map(
          _.flow(
            ...
            _.set('address', scriptHash2addr[scriptHash]),
          ),
          result)))))
}
 ...
 * @param inputs An array of UTXOs: [{ txId, vout, address, value }, ...]
 ...
 */
const createTransaction = (inputs, outputs, feeRate, network) => {
  const UTXOKey = ({ txId, vout }) => JSON.stringify([txId, vout])

  // Ignore UTXOs that we've used in the past ~1min
  inputs = _.filter(utxo => !recentlyUsedUTXOs.has(UTXOKey(utxo)), inputs)
  ...
  return PromiseObject({ inputs, outputs })
    .then(({ inputs, outputs }) => {
      ...
      _.flow(
        _.map(utxo => ({ key: UTXOKey(utxo), val: true })),
        kvs => recentlyUsedUTXOs.mset(kvs),
      )(selected.inputs)
