...
const filterUnconsumedIndices = consumedIndices => _.flow(
  _.update('internal', all => _.difference(all, consumedIndices.internal)),
  _.update('external', all => _.difference(all, consumedIndices.external)),
  spy("filterUnconsumedIndices():ret:"),
)

const nonConsumedIndices = allIndices =>
  getConsumedIndices(['internal', 'external'])
    .then(consumedIndices => _.flow(
      enumerateIndices,
      filterUnconsumedIndices(consumedIndices),
    )(allIndices))
...
const updateTables = (utxos, selectedInputs) => {
  const extractAddresses = _.flow(_.map(_.pick(['addressIndex', 'addressType'])), _.uniq)
  ...
  const newlyConsumedAddresses = _.difference(addressesBefore, addressesAfter)
  setConsumedIndices(newlyConsumedAddresses)
...
const checkConsumedAddresses = () =>
  PromiseObject({
    network: networkPromise,
    indices: getConsumedIndices(['internal', 'external']),
  })
  .then(({ network, indices }) => fetchUTXOsForIndices(network)(indices))
  .then(_.flow(
