...
const setEmptyAddress = (type, idx) => EMPTY_INDICES[type] = _.set([idx], true, EMPTY_INDICES[type])
const isEmptyAddress = (type, idx) => _.get([type, idx], EMPTY_INDICES)
const unsetEmptyAddress = (type, idx) => EMPTY_INDICES[type] = _.unset([idx], EMPTY_INDICES[type])

const allIndices = ({ internal, external }) => ({
  internal: _.range(0, internal + GAP_LIMIT),
  external: _.range(0, external + GAP_LIMIT),
})

const filterEmptyIndices = type => _.filter(idx => !isEmptyAddress(type, idx))

const nonEmptyIndices = _.flow(
  allIndices,
  _.update('internal', filterEmptyIndices('internal')),
  _.update('external', filterEmptyIndices('external')),
)

const fetchAllUTXOs = network => freeAddressIndices()
  .then(_.flow(nonEmptyIndices, fetchUTXOsForIndices(network)))

const fetchUTXOsForIndices = network => indices => _.flow(
...
.then(utxos => {
  console.debug("EMPTY_INDICES:", EMPTY_INDICES)
...
const updateTables = (utxos, selectedInputs) => {
  const extractAddresses = _.flow(_.map(_.pick(['address', 'addressIndex', 'addressType'])), _.uniq)
  const isSameUTXO = (a, b) => a.txId === b.txId && a.vout === b.vout

  const addressesBefore = extractAddresses(utxos)
  const addressesAfter = extractAddresses(_.differenceWith(isSameUTXO, utxos, selectedInputs))

  const newlyEmptiedAddresses = _.difference(addressesBefore, addressesAfter)
  _.forEach(
    ({ addressType, addressIndex }) => setEmptyAddress(addressType, addressIndex),
    newlyEmptiedAddresses
  )
...
const createTransaction = (utxos, outputs, feeRate, network) => {
  ...
  let inputs = _.filter(utxo => !recentlyUsedUTXOs.has(UTXOKey(utxo)), utxos)
...
const checkEmptyAddresses = () =>
  networkPromise
    .then(network => _.flow(
        _.update('internal', _.keys),
        _.update('external', _.keys),
        fetchUTXOsForIndices(network)
      )(EMPTY_INDICES)
    )
    .then(_.flow(
      _.map(_.pick(['addressType', 'addressIndex'])),
      _.uniq,
      _.forEach(({ addressType, addressIndex }) => unsetEmptyAddress(addressType, addressIndex)),
    ))
