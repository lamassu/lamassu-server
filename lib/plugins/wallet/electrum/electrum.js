...
const freeAddressIndices = (types=['internal', 'external']) =>
  helpers.freeAddressIndices('BTC', wallet_hash, types)

/* Retrieves the next {in,ex}ternal free index */
const _nextFreeAddressIndex = type => () => freeAddressIndices([type]).then(_.get(type))
...
const filterUnconsumedIndices = (consumed, { internal, external }) => ({
  'internal': _.difference(internal, consumed.internal),
  'external': _.difference(external, consumed.external),
})

const nonConsumedIndices = () =>
  PromiseObject({
    nextIndices: freeAddressIndices(),
    consumedIndices: getConsumedIndices(['internal', 'external']),
  })
  .then(({ nextIndices, consumedIndices }) =>
    filterUnconsumedIndices(consumedIndices, enumerateIndices(nextIndices))
  )

const fetchUTXOs = network => nonConsumedIndices().then(fetchUTXOsForIndices(network))

const indicesToAddressInfos = network => _.flow(
...
const fetchUTXOsForIndices = network => _.flow(
  indicesToAddressInfos(network),
  getUTXOsBatch(network),
)

const fetchConfirmedUTXOs = network => fetchUTXOs(network).then(_.filter(_.get('confirmed')))
const fetchUnconfirmedUTXOs = network => fetchUTXOs(network).then(_.remove(_.get('confirmed')))

const getBalanceBatch = network => _.flow(
  _.map(_.flow(_.get('address'), scriptHashFromAddress(network))),
  scriptHashes =>
    electrumPromise
      .then(electrum =>
        Promise.all(_.flow(
          // Balance replies are smaller than UTXOs replies, so it should be
          // safe to ask for more than the usual amount
          _.chunk(2*CHUNK_SIZE),
          _.map(chunk => electrum.blockchainScripthash_getBalanceBatch(chunk)),
        )(scriptHashes)))
      .then(_.flow(
        _.flatten,
        _.map(_.get('result')),
        _.reduce(
          (acc, res) => ({
            confirmed: acc.confirmed.plus(res.confirmed),
            unconfirmed: acc.unconfirmed.plus(res.unconfirmed),
          }),
          { confirmed: new BN(0), unconfirmed: new BN(0) }
        ),
      ))
)

const getAllBalance = network =>
  nonConsumedIndices()
    .then(indicesToAddressInfos(network))
    .then(getBalanceBatch(network))

const getConfirmedBalance = network => getAllBalance(network).then(_.get('confirmed'))
const getUnconfirmedBalance = network => getAllBalance(network).then(_.get('unconfirmed'))
...
const newFunding = (account, cryptoCode, settings, operatorId) =>
  checkCryptoCode(cryptoCode)
    .then(() => networkPromise)
    .then(getAllBalance)
    .then(({ confirmed, unconfirmed }) => PromiseObject({
      fundingPendingBalance: unconfirmed,
      fundingConfirmedBalance: confirmed,
