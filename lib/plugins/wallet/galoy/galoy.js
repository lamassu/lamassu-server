...
const invoiceLN = require('@node-lightning/invoice')
...
function sendFundsLN (walletId, invoice, cryptoAtoms, invoiceAmount, token, endpoint) {
  const sendLn = {
  ...
  if (_.isNull(invoiceAmount)) return request(sendLnNoAmount, token, endpoint).then(result => result.data.lnNoAmountInvoicePaymentSend)
  if (!cryptoAtoms.eq(new BN(invoiceAmount))) throw new Error(`Invoice amount doesn't match the amount inserted!`)
  return request(sendLn, token, endpoint).then(result => result.data.lnInvoicePaymentSend)
}

function sendProbeRequest (walletId, invoice, invoiceAmount, cryptoAtoms, token, endpoint) {
  const sendProbe = {
  ...
  if (_.isNull(invoiceAmount)) return request(sendProbeNoAmount, token, endpoint).then(result => result.data.lnNoAmountInvoiceFeeProbe)
  return request(sendProbe, token, endpoint).then(result => result.data.lnInvoiceFeeProbe)
}
  ...
  return checkCryptoCode(cryptoCode)
    ...
    .then(galoyAccount => {
      ...
      if (isLightning(toAddress)) {
        const invoiceAmount = invoiceLN.decode(toAddress)._value // TODO: Millisatoshis, requires convertion?
        return sendFundsLN(wallet.id, toAddress, cryptoAtoms, invoiceAmount, account.apiSecret, account.endpoint)
      }
...
function probeLN (account, cryptoCode, invoice) {
  ...
  const invoiceAmount = invoiceLN.decode(invoice)._value // TODO: Millisatoshis, requires convertion?
  if (!_.isNull(invoiceAmount)) {
    // TODO: not available for non-zero amount invoices
  }
  const results = probeHardLimits.map(limit => {
    return sendProbeRequest(account.walletId, account.apiSecret, account.endpoint, invoice, invoiceAmount, limit)
      .then(r => _.isEmpty(r.erros))
  })
  return _.zipObject(results, probeHardLimits)
}
