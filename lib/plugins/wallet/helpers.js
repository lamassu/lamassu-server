...
const formatTuple = arr => '('+pgp.as.csv(arr)+')'
 ...
 */
const setConsumedIndices = (cryptoCode, walletHash, indices) => {
  const now = new Date()
  return _.flow(
    _.map(({ addressType, addressIndex }) => formatTuple([cryptoCode, walletHash, addressType, addressIndex, now])),
    _.join(','),
    rows => db.none(
      `INSERT INTO wallet_consumed_indices (crypto_code, wallet_hash, type, index, last_check)
      VALUES $1^
      ON CONFLICT (crypto_code, wallet_hash, type, index) DO
      UPDATE SET last_check = $2;`,
      [rows, now]
    ),
  )(indices)
}
...
const unsetConsumedIndices = (cryptoCode, walletHash, indices) => _.flow(
  _.map(({ addressType, addressIndex }) => formatTuple([addressType, addressIndex])),
  _.join(','),
  typeIndexPairs => db.none(
    ...
    WHERE crypto_code = $1
      AND wallet_hash = $2
      AND (type, index) IN ($3^);`,
    [cryptoCode, walletHash, typeIndexPairs]
  ),
)(indices)
...
/*
 * @brief Similar to @see getConsumedIndices but with limitted number of rows
 */
const getConsumedIndicesChunk = (cryptoCode, walletHash, types, chunkSize) =>
  db.any(
    `SELECT index, type
    FROM wallet_consumed_indices
    WHERE crypto_code = $1
    ...
    LIMIT $4;`,
    [cryptoCode, walletHash, formatTypes(types), chunkSize]
  )
  .then(_.flow(
    _.groupBy(_.get('type')),
    _.mapValues(_.map(_.get('index'))),
