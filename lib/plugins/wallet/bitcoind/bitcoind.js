...
function newAddress (account, info, tx, settings, operatorId) {
  return checkCryptoCode(info.cryptoCode)
    .then(() => {
      if (_.isNil(info.funding)) return fetch('getnewaddress')
      return fetch('getnewaddress', [`funding-${Date.now()}`])
    })
    .catch(errorHandle)
}
  ...
  return checkCryptoCode(cryptoCode)
    .then(() => {
      const promises = [
        ...
        newAddress(account, { cryptoCode, funding: true }),
        getWalletHistory(cryptoCode)
      ]
...
function getWalletHistory (cryptoCode) {
  return checkCryptoCode(cryptoCode)
    ...
    .then(res => {
      return _.map(it => {
        if (it.category === 'send') return _.assign({ operation: 'cash-in' }, it)
        else if (it.category === 'receive' && _.isEmpty(it.label)) return _.assign({ operation: 'cash-out' }, it)
        else if (it.category === 'receive' && _.includes('funding', it.label)) return _.assign({ operation: 'funding' }, it)
        else if (it.category === 'generate' || it.category === 'immature') return _.assign({ operation: 'coinbase' }, it)
        return it
      }, res)
    })
}
