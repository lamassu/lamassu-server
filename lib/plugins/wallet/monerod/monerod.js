...
function newFunding (account, cryptoCode, settings, operatorId, cashOutAddresses) {
  return checkCryptoCode(cryptoCode)
    ...
    .then(([balanceRes, addressRes, transferRes, walletHistory]) => {
...
function getWalletHistory (cryptoCode, cashOutAddresses) {
  return checkCryptoCode(cryptoCode)
    .then(() => fetch('get_transfers', { in: true, out: true, account_index: 0 }))
    .then(res => {
      const list = _.orderBy(it => it.timestamp, 'desc')([...res.in, ...res.out])

      return _.map(it => {
        if (it.type === 'out') return { ...row, amount: Math.abs(it.amount), operation: 'cash-in' }
        else if (it.type === 'in' && !_.includes(it.address, cashOutAddresses)) return { ...row, operation: 'funding' }
        else if (it.type === 'in') return { ...row, operation: 'cash-out' }
        return it
      }, list)
    })
}
