...
function sendCoins (account, tx, settings, operatorId, feeMultiplier) {
  logger.info(`[XMR] Entering sendCoins at ${new Date().toISOString()}`)
  const { toAddress, cryptoAtoms, cryptoCode } = tx
  return checkCryptoCode(cryptoCode)
    .then(() => {
      logger.info(`[XMR] Entering transfer_split at ${new Date().toISOString()}`)
      return fetch('transfer_split', {
    ...
    .then(res => {
      logger.info(`[XMR] Leaving transfer_split and finishing sendCoins at ${new Date().toISOString()}`)
      return {
