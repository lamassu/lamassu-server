...
class CardanoApi {
  ...
  sumAmountByAddress(movements, address) {
    return movements
      ...
      .reduce((acc, amount) => acc.add(amount), BC(0))
  }
      ...
      .then(transactions =>
        transactions
          .map(transaction => {
            ...
            if (mode & CardanoApi.INCLUDE_INCOME_TRANSACTIONS) {
              amount.minus(this.sumAmountByAddress(transaction.inputs, address))
            }

            if (mode & CardanoApi.INCLUDE_OUTCOME_TRANSACTIONS) {
              amount.plus(this.sumAmountByAddress(transaction.output, address))
            }
...
CardanoApi.INCLUDE_INCOME_TRANSACTIONS = 1
CardanoApi.INCLUDE_OUTCOME_TRANSACTIONS = 2
