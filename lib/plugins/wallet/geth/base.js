  ...
  return checkCryptoCode(cryptoCode)
    .then(code => {
      ...
      const promises = [
        ...
        getWalletHistory(account, cryptoCode, cashOutAddresses)
      ]

      return Promise.all(promises)
        .then(([fundingPendingBalance, fundingConfirmedBalance, walletHistory]) => ({
...
function getWalletHistory (account, cryptoCode, cashOutAddresses) {
  return checkCryptoCode(cryptoCode)
    .then(() => {
