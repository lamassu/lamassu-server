...
function sendCoins (account, tx, settings, operatorId) {
  ...
  return generateTx(toAddress, defaultWallet(account), cryptoAtoms, false, cryptoCode)
    .then(signedTx => {
      console.log(`** DEBUG ** Signed transaction ${signedTx}`)
      return signedTx
    })
    .then(pify(web3.eth.sendSignedTransaction))
    .then(txid => {
      console.log(`** DEBUG ** Transaction hash ${txid}`)
      return txid
    })
    .then(txid => {
      return pify(web3.eth.getTransaction)(txid)
        .then(tx => {
          console.log(`** DEBUG ** web3.eth.getTransaction output: ${JSON.stringify(tx)}`)
          pify(web3.eth.getPendingTransactions)().then(pendingTxs => console.log(`** DEBUG ** Pending transactions: ${pendingTxs}`))
          if (!tx) return { txid }
...
function generateTx (_toAddress, wallet, amount, includesFee, cryptoCode) {
  ...
  return Promise.all(promises)
    ...
    .then(([gas, gasPrice, txCount, baseFeePerGas]) => {
      ...
      console.log(`** DEBUG ** Transaction object ${JSON.stringify(rawTx)}`)
