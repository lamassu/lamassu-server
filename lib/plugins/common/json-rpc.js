...
function rpcConfig (cryptoRec) {
  try {
    ...
    return {
      username: isRemoteWallet(cryptoRec) ? process.env[`${cryptoRec.cryptoCode}_NODE_USER`] : config.rpcuser,
      password: isRemoteWallet(cryptoRec) ? process.env[`${cryptoRec.cryptoCode}_NODE_PASSWORD`] : config.rpcpassword,
      host: isRemoteNode(cryptoRec) ? process.env[`${cryptoRec.cryptoCode}_NODE_HOST`] : 'localhost',
      port: isRemoteNode(cryptoRec) ? process.env[`${cryptoRec.cryptoCode}_NODE_PORT`] : config.rpcport || cryptoRec.defaultPort
    }
  } catch (err) {
