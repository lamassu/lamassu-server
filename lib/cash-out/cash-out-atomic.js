  ...
  return Promise.resolve(updateStatus(oldTx, newTx))
    .then(updatedTx => {
      if (updatedTx.status !== oldTx.status) {
        updatedTx.justAuthorized = wasJustAuthorized(oldTx, updatedTx)
        const rec = {
...
function wasJustAuthorized (oldTx, newTx) {
  const isPublished = _.includes(oldTx.status, ['notSeen', 'rejected']) &&
    _.includes(newTx.status, ['published', 'authorized', 'instant', 'confirmed'])
