...
function cashboxRemoved (deviceId, deviceTime) {
  const eventId = uuidv4()

  const event = {
    id: eventId,
    deviceId: deviceId,
    eventType: 'cashboxRemoved',
    deviceTime: deviceTime
  }
  return dbm.machineEvent(event)
}

function cashboxInserted (deviceId, deviceTime, rec) {
  const eventId = uuidv4()

  const event = {
    id: eventId,
    deviceId: deviceId,
    eventType: 'cashboxInserted',
    deviceTime: deviceTime,
    cashboxSessionId: rec.uuid
  }
  const session = {
    id: rec.uuid,
    deviceId: deviceId,
  ...
  return dbm.machineEvent(event).then(() => dbm.cashboxSessionInsert(session))
}

function cashboxInitialize (deviceId, deviceTime) {
  const session = {
    id: uuidv4(),
    deviceId: deviceId,
    deviceTime: deviceTime
  ...
  return dbm.cashboxSessionInsert(session)
}
