...
const selectFromNamespace = _.curry((key, config) => filter(key)(config))
...
const getCommissions = (cryptoCode, deviceId, config) => {
  const commissions = fromNamespace(CONFIG_NAMESPACES.COMMISSIONS)(config)
...
const getLocale = (deviceId, it) => {
  const locale = fromNamespace(CONFIG_NAMESPACES.LOCALE)(it)
...
const getWalletSettings = (key, it) => {
  ...
  const getAdvancedSettings = it => {
    const advancedSettings = fromNamespace(CONFIG_NAMESPACES.ADVANCED)(it)
    return _.omit(['overrides', 'cryptoCurrency', 'id'], _.assignAll([advancedSettings, ..._.filter(filter)(advancedSettings.overrides)]))
  }
  const walletsSettings = fromNamespace(CONFIG_NAMESPACES.WALLETS)(it)
  return _.assign(fromNamespace(key)(walletsSettings), getAdvancedSettings(walletsSettings))
}

const getCashOut = (key, it) => _.compose(fromNamespace(key), fromNamespace(CONFIG_NAMESPACES.CASH_OUT))(it)
const getGlobalCashOut = fromNamespace(CONFIG_NAMESPACES.CASH_OUT)
const getOperatorInfo = fromNamespace(CONFIG_NAMESPACES.OPERATOR_INFO)
const getCoinAtmRadar = fromNamespace(CONFIG_NAMESPACES.COIN_ATM_RADAR)
const getTermsConditions = fromNamespace(CONFIG_NAMESPACES.TERMS_CONDITIONS)
const getReceipt = fromNamespace(CONFIG_NAMESPACES.RECEIPT)
...
const pickTermsAndConditions = selectFromNamespace(CONFIG_NAMESPACES.TERMS_CONDITIONS)
const pickGlobalCashIn = selectFromNamespace(CONFIG_NAMESPACES.CASH_IN)
const pickGlobalCashOut = selectFromNamespace(CONFIG_NAMESPACES.CASH_OUT)
const pickCompliance = selectFromNamespace(CONFIG_NAMESPACES.COMPLIANCE)
// Triggers are stored in the 'triggers' field, where the namespace has the same name as the field, which does not occur in other namespaces
const pickTriggers = _.pick([CONFIG_NAMESPACES.TRIGGERS])
const pickTriggersConfig = selectFromNamespace(CONFIG_NAMESPACES.TRIGGERS_CONFIG)

const getAllCryptoCurrencies = (config) => {
  const locale = fromNamespace(CONFIG_NAMESPACES.LOCALE)(config)
...
const getNotifications = (cryptoCurrency, machine, config) => {
  const notifications = fromNamespace(CONFIG_NAMESPACES.NOTIFICATIONS)(config)
...
const getCashInSettings = config => fromNamespace(CONFIG_NAMESPACES.CASH_IN)(config)

const getCryptoUnits = (crypto, config) => getWalletSettings(crypto, config).cryptoUnits

const setTermsConditions = toNamespace(CONFIG_NAMESPACES.TERMS_CONDITIONS)
