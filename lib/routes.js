...
function getLastSeen (req, res, next) {
  ...
  // Make it run the first time around
  if (!canGetLastSeenMap[deviceId]) {
    canGetLastSeenMap[deviceId] = timestamp - THROTTLE_LOGS_QUERY - 1
  }

  if (timestamp - canGetLastSeenMap[deviceId] > THROTTLE_LOGS_QUERY) {
...
function filterOldRequests (req, res, next) {
  ...
  // Make it run the first time around
  if (!canLogClockSkewMap[deviceId]) {
    canGetLastSeenMap[deviceId] = timestamp - THROTTLE_CLOCK_SKEW - 1
  }

  if (delta > CLOCK_SKEW && timestamp - canLogClockSkewMap[deviceId] > THROTTLE_CLOCK_SKEW) {
