...
function cashboxRemoved (req, res, next) {
  helpers.cashboxRemoved(req.deviceId, req.deviceTime)
    .then(() => respond(req, res))
    .catch(next)
}

function cashboxInserted (req, res, next) {
  helpers.cashboxInserted(req.deviceId, req.deviceTime, req.body)
    .then(() => respond(req, res))
    .catch(next)
}
    ...
    .then(valid => {
      if (valid) {
        return helpers.updateMachineDefaults(deviceId)
          .then(() => helpers.cashboxInitialize(deviceId, deviceTime))
...
app.post('cashbox_removed', cashboxRemoved)
app.post('cashbox_inserted', cashboxInserted)
