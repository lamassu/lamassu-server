...
const THROTTLE_LOGS_QUERY = 5 * 60 * 1000
const THROTTLE_CLOCK_SKEW = 1 * 60 * 1000
...
_.mixin({
  memoizeThrottle: function(wait=0, func) {
    var mem = _.memoize(function(param) {
      return _.throttle(wait, func)
    });
...
function getLastSeen (req, res, next) {
  const deviceId = req.deviceId
  const runObject = { canRun: false }
  shouldRunGetLastSeen(deviceId, runObject)

  if (canGetLastSeenMap[deviceId]) {
    canGetLastSeenMap[deviceId] = false
    return logs.getLastSeen(deviceId)
      .then(r => res.json(r))
      .catch(next)
  }

  // force timeout to avoid machine log dumps
  return res.status(403).json({})
}

const shouldRunGetLastSeen = _.memoizeThrottle(THROTTLE_LOGS_QUERY, deviceId => {
  canGetLastSeenMap[deviceId] = true
})
...
function filterOldRequests (req, res, next) {
  ...
  shouldLogClockSkew(req.deviceId)

  if (delta > CLOCK_SKEW && canLogClockSkewMap[deviceId]) {
    canLogClockSkewMap[deviceId] = false
...
const shouldLogClockSkew = _.memoizeThrottle(THROTTLE_CLOCK_SKEW, deviceId => {
  canLogClockSkewMap[deviceId] = true
})
