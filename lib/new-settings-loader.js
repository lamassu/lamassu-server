...
function saveAccounts (accounts, shouldMerge = true) {
  return Promise.all([loadAccounts(), getOperatorId('middleware')])
    .then(([currentAccounts, operatorId]) => {
      const newAccounts = shouldMerge ? _.mergeWith((a, b) => _.isArray(b) ? b : undefined, currentAccounts, accounts) : accounts
      return db.tx(t => {
...
function resetAccount (accountId, instanceId, schemaVersion = NEW_SETTINGS_LOADER_SCHEMA_VERSION) {
  return loadAccounts(schemaVersion)
    .then(accs => {
      const isLastInstance = _.size(accs[accountId].instances) === 1 && accs[accountId].instances[0].id === instanceId
      return db.none(
        accountsSql,
        [
          'accounts',
          isLastInstance
            ...
            : { accounts: { ...accs, [accountId]: { ...accs[accountId], instances: _.remove(it => it.id === instanceId, accs[accountId].instances) } } },
          true,
          schemaVersion
        ]
      )
