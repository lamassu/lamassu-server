...
const { insertDevice, removeDevice } = require('./ultralight/queries')
...
function unpair (deviceId) {
  return db.tx(t => 
    ...
    .then(() => {
        ...
        return removeDevice(deviceId).then(() => {
...
function pair (operatorId, token, deviceId, machineModel, numOfCassettes = DEFAULT_NUMBER_OF_CASSETTES, numOfStackers = DEFAULT_NUMBER_OF_STACKERS) {
  return pullToken(token)
    ...
    do update set paired=TRUE, display=TRUE`
      ...
      return db.none(insertSql, [deviceId, r.name, numOfCassettes, numOfStackers])
        .then(() => insertDevice(deviceId, operatorId))
        .then(() => true)
    })
    .catch(err => {
      logger.debug(err)
